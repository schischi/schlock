...
t_lock *win_create(char *msg)
{
    ...
    if (msg) {
        ...
        lock->msg_len = asprintf(&lock->msg, "This screen is locked by %s",
                getenv("USER"));
    }
    lock->failure = 0;
...
void win_str(t_lock *lock, t_buf *buf)
{
    ...
    if (lock->failure) {
        char fail_buf[255];
        snprintf(fail_buf, 255, "Failures: %d", lock->failure);
        int fail_len = strlen(fail_buf);
        XDrawString(lock->display, lock->win, lock->gc, 10, 80, fail_buf, fail_len);
    }
    XFlush(lock->display);
}
